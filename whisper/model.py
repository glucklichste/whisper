...
class MultiHeadAttention(nn.Cell):
    def __init__(self, n_state: int, n_head: int):
        ...
        self.key = nn.Dense(n_state, n_state, has_bias=False)
...
class Whisper(nn.Module):
    ...
    def set_alignment_heads(self, dump: bytes):
        ...
        mask = ms.from_numpy(array).reshape(
            self.dims.n_text_layer, self.dims.n_text_head
        )
        self.register_buffer("alignment_heads", mask.to_sparse(), persistent=False)

    def embed_audio(self, mel: ms.Tensor):
        return self.encoder(mel)

    def logits(self, tokens: ms.Tensor, audio_features: ms.Tensor):
        return self.decoder(tokens, audio_features)

    def construct(
        self, mel: ms.Tensor, tokens: ms.Tensor
    ) -> Dict[str, ms.Tensor]:
